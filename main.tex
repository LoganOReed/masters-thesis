...
\section{Background}
\todo[inline]{Talk about symmetrizations and maybe move the specific eigenvalue examples here}
\todo[inline]{Should have Disk, rect, Tri, Pólya showing two cases and the rest being open}.
...
\todo[inline]{Paste Polya conjecture here}
...
\todo[inline]{Copy this file from seminar class folder}
...
\todo[inline]{Should I put the explicit eigenvalues examples here or elsewhere?}
...
We will denote by $A_{L}^{D}(\Omega)$ the linear operator defined by 

\begin{align*}
  A_{L}^{D} : L^{2}(\Omega) &\to H_{0}^{1}(\Omega) \subset L^{2}(\Omega), \\
...
\todo[inline]{Write overview of how to choose $(x_{i}), (y_{i})$ or refer to various papers and summarize?}
...
\todo[inline]{Should I include a basic derivation of the runtime? That is more of a CS thing, so I'm not sure that it is appropriate to add.}
