...
We begin by proving a result due to Henrot \cite{henrot}.
...
\todo[inline]{Split proof into at least one additional theorem}
...
\begin{proof}
  ...
  We begin with a result similar to Theorem \ref{eqmin} (due to Bogosel \cite{bogosel}) which shows that the minimization $\min \left\{ \lambda_{n}(\Omega); |\Omega| = c \right\} $ is equivalent (up to homothety) of the following minimization problem 
  \[
