...
% Consider a homogeneous elastic membrane which is stretched and bounded to a rigid frame.
% We will represent the frame as a two dimensional domain $\Omega \subset \mathbb{R}^{2} $.
% Take $t > 0$ and $x = \left( x_1, x_2 \right)  \in \Omega$.
% Let $u = u(x,t) : \overline{\Omega} \times [0,\infty) \to \mathbb{R}^{}  $ be the orthogonal displacement from the equilibrium position $x$ at time $t$, and assume the parallel displacement is negligible.
%
% For any smooth subregion $V$ of $\Omega$, the acceleration of the displacement within $V$ is
%
% \[
% \D[2]{}{t} \int_V \! u \, \mathrm{d}x = \int_V \! u_{tt} \, \mathrm{d}x 
% .\] 
%
% Further, the net contact force will be
%
% \[
%   - \int_{\partial V} \! \textbf{F} \cdot \textbf{n} \, \mathrm{d}S 
% ,\] 
%
% where $\textbf{F}$ is the force acting upon $V$ through $\partial V$ and the mass density is, without loss of generality, taken to be the unit.
% It then follows directly from Newton's second law of motion that
%
% \[
% \int_V \! u_{tt} \, \mathrm{d}x = - \int_{\partial V} \! \textbf{F} \cdot \textbf{n} \, \mathrm{d}S  
% .\] 
% TODO Come back to Evans version and look into last approximation
