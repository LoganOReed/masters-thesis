...
\todo[inline]{The best reference I could find is Logan's Applied Partial Differential Equations.}
...
\section{Elementary Sobolev Space Theory}
...
\begin{proof}
\todo[inline]{Pg 335 Evans}
\todo[inline]{Ask Fu about elementary proof for first statement}


\end{proof}

\todo[inline]{pg 336 Evans has an alternative method of defining eigenvalues of elliptic operators}

\break

\section{Known Results}
\todo[inline]{invariant under translations rotations}
...
\todo[inline]{should I show more steps?}
...
\begin{proof}
  This proof uses the same argument as the proof for the Faber-Krahn Theorem, but we will now use Steiner symmetrization in lieu of the Schwarz rearrangement.
  As the Steiner symmetrization shares the properties \ref{fk1} and \ref{fk2}, we know that any Steiner symmetrization will not increase the first eigenvalue.
  ...
  Note that $\frac{\sqrt{3}}{2}$ is the value characteristic of equilateral triangles.

  \todo[inline]{Ask Fu if gamma-convergence and the like are necessary to explain and in what detail}
  \todo[inline]{finish proof}
...
In this section we will demonstrate two domains over which we can obtain the eigenvalues of the Dirichlet Laplacian explicitly. 
We will begin with a rectangular domain, defined by
