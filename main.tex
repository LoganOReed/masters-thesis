...
Pólya-Szego went on to show that the second Dirichlet eigenvalue of the Laplacian is minimized by two balls under volume constraint.
...
The majority of these problems can be shown to be equivalent, up to some homothety.
...
Showing the first two properties hold for the Laplacian is straightforward and elementary, and we can show the Laplacian is compact as an immediate consequence of the Rellich Theorem(\ref{rellich}).
...
We will begin with foundational properties of the Dirichlet eigenvalues
...
\todo[inline]{Use invariance of laplacian under orthogonal stuff to show invariance of eigenvalue}
\todo[inline]{Add monotonicity resutl}
...
In this paper we will use the Method of Fundamental Solutions to solve the direct problem\cite{bogomolny}\cite{fund}, while the optimization procedure is performed with a standard gradient method\cite{snyman}\cite{wright}.

\todo[inline]{Mention method for generating initial domain}
\todo[inline]{Mention potentially using genetic algorithm}
